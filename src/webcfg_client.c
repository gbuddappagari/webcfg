...
int send_aker_blob(char *blob, uint16_t docTransId, int version)
{
	...
	while(1)
	{
		...
		if(wrp_msg != NULL)
		{
			...
			if (wrp_msg->msg_type == WRP_MSG_TYPE__UPDATE)
			{
				...
				{
					WebcfgInfo("Update response received from %s with transaction_uuid %s\n",sourceService, wrp_msg->u.crud.transaction_uuid );
